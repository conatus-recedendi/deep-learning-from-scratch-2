    ...
    if corpus.ndim == 1:
        ...
        for idx_0, word_ids in enumerate(corpus):
            for idx_1, word_id in enumerate(word_ids):
                one_hot[idx_0, idx_1, word_id - 1] = 1
...
def create_contexts_target(corpus, window_size=1):
    ...
    for idx, id in enumerate(corpus):
        ...
        if idx < window_size:
            continue
        if idx >= len(corpus) - window_size:
            continue

        target.append(corpus[idx])
        # excpet corpus[idx]
        for i in range(1, window_size + 1):
            temp_contexts.append(corpus[idx - i])
            temp_contexts.append(corpus[idx + i])

        contexts.append(temp_contexts)
        # contexts = np.append(contexts, except_corpus)
...
def cast_to_single_value(x):
    """
    :param x: zero-dim(for GPU) or single value(for CPU)
    return: casted value
    """
